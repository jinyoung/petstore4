...
func (self *StoreOrder) Get(c echo.Context) error {
	repository := StoreOrderRepository()
	entities := repository.GetList()
	return c.JSON(http.StatusOK, entities)
}

func (self *StoreOrder) GetbyId(c echo.Context) error{
	repository := StoreOrderRepository()
...
func (self *StoreOrder) Persist(c echo.Context) error{
	repository := StoreOrderRepository()
...
func (self *StoreOrder) Put(c echo.Context) error{
	repository := StoreOrderRepository()
	id, _ := strconv.Atoi(c.Param("id"))
	params := make(map[string] string)
...
func (self *StoreOrder) Remove(c echo.Context) error{
	repository := StoreOrderRepository()
	id, _ := strconv.Atoi(c.Param("id"))
	self = repository.GetID(id)
