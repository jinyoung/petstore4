...
func (self *Menu) Get(c echo.Context) error {
	repository := MenuRepository()
	entities := repository.GetList()
	return c.JSON(http.StatusOK, entities)
}

func (self *Menu) GetbyId(c echo.Context) error{
	repository := MenuRepository()
...
func (self *Menu) Persist(c echo.Context) error{
	repository := MenuRepository()
...
func (self *Menu) Put(c echo.Context) error{
	repository := MenuRepository()
	id, _ := strconv.Atoi(c.Param("id"))
	params := make(map[string] string)
...
func (self *Menu) Remove(c echo.Context) error{
	repository := MenuRepository()
	id, _ := strconv.Atoi(c.Param("id"))
	self = repository.GetID(id)
